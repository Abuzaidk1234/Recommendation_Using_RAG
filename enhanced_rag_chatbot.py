...
if not api_key:
    print("WARNING: GEMINI_API_KEY environment variable not set. RAG features will not work.")
else:
...
def generate_with_fallback(prompt: str):
    """Attempt generation with fallback models if rate limits are hit"""
    last_error = None
    log_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fallback_debug.log')
    
    with open(log_file, 'a', encoding='utf-8') as f:
        f.write(f"\\n--- New Generation Request ---\\n")
        for model_name in FALLBACK_MODELS:
            try:
                f.write(f"Attempting {model_name}... ")
                current_model = genai.GenerativeModel(model_name)
                response = current_model.generate_content(prompt)
                
                # Check for safety blocks or missing text
                try:
                    text = response.text
                    if text:
                        f.write("SUCCESS\\n")
                    ...
                    f.write(f"FAILED (No valid text, possibly blocked by safety filters: {ve})\\n")
                    last_error = ve
                    continue
                ...
                if "429" in error_str or "quota" in error_str or "exhausted" in error_str:
                    print(f"[!] Rate limit hit for {model_name}. Trying next...")
                    f.write(f"RATE LIMITED\\n")
                    last_error = e
                    continue
                else:
                    print(f"[!] Model error for {model_name}: {e}. Trying next...")
    ...
    try:
        ...
        try:
            if os.environ.get("GEMINI_API_KEY"):
                ...
                print("❌ LLM connection failed: GEMINI_API_KEY not set")
                return
        except Exception as e:
...
def generate_llm_rag_response(query: str, search_results: List[Dict]) -> str:
    ...
    try:
        ...
        prompt = f'''You are an intelligent food recommendation assistant. The user is asking a question or making a request about food, and I've retrieved relevant context from a food database.
...
Response:'''
        ...
        # Generate response using Google Gemini with fallback
        generated_response = generate_with_fallback(prompt)
...
def generate_llm_comparison(query1: str, query2: str, results1: List[Dict], results2: List[Dict]) -> str:
    ...
    try:
        ...
        comparison_prompt = f'''You are analyzing and comparing two different food preference queries. Please provide a concise comparison formatted STRICTLY as a Markdown table. Do not write lengthy paragraphs.
...
Instructions:
1. Output ONLY a neat Markdown table.
2. Use columns: "Feature", "{query1}", "{query2}".
3. Use rows: "Top Match", "Cuisine", "Calories", "Best For", "Key Difference".
4. Keep the text in each cell very brief and concise (1-2 short sentences maximum).

Markdown Table:'''

        generated_response = generate_with_fallback(comparison_prompt)
